...
## A — UI 구현 (mock 데이터) `[MVP]` ← 현재 단계 (진행 중)
...
- ✅ **디자인 토큰 단일화**: 색상 단일 출처 `theme/colors.js` 도입(`tailwind.config.js`가 require, 인라인 style은 `import {colors}`로 공유) → 컴포넌트 hex 하드코딩 전부 토큰으로 교체. `cn()`을 `clsx + tailwind-merge`로 교체해 컴포넌트 기본 클래스 오버라이드 가능.
- 🔲 **디자인 다듬기**: 앨범·설정 탭 세부 디자인 조정 (진행 중 — 앨범 월별 리스트 보더·여백·이미지 크기 조정 완료).
