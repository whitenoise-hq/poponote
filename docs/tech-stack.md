...
### 2.3 UI
- **스타일링: NativeWind (Tailwind CSS for React Native)** — `className` 문법 사용.
  - 웹 Tailwind와 거의 동일한 사용성. 단, RN 특성상 일부 웹 전용 클래스(일부 hover/grid 등)는 제한될 수 있음.
  - 디자인 토큰(색상·타이포·간격)은 `tailwind.config.js`에 정의해 일관성 유지.
...
### 3.1 인증 (Auth)
- **카카오 / 구글 OAuth 간편로그인** 사용 (Supabase Auth provider 설정).
...
- **포맷**: WebP 우선 (동일 화질 대비 용량 작음, 특히 일러스트처럼 단순한 이미지에 효율적).
...
- 처리 흐름:
  1. RN: 업로드 전 사진을 리사이즈·압축(WebP) → 압축본 업로드, 변환용으로 원본(또는 고화질본)도 임시 전달
...
3. 카카오/구글 OAuth provider 설정 및 로그인 플로우 구현
