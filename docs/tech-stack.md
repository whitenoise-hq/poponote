...
| 영역 | 선택 | 비고 |
|------|------|------|
| 프론트엔드 | React Native (Expo 권장) | 크로스플랫폼 프레임워크 · **iOS(App Store) 전용 배포**, Android 미지원 |
| 언어 | TypeScript | |
| 백엔드 | Supabase | DB·인증·스토리지·서버리스 함수 통합 |
| 데이터베이스 | Supabase Postgres | |
| 인증 | Supabase Auth (카카오 OAuth + Sign in with Apple) | Apple 로그인은 iOS 배포 가이드라인 4.8 충족용 필수. 구글은 MVP 범위 밖 |
...
### 3.1 인증 (Auth)
- **카카오 OAuth 간편로그인** + **Sign in with Apple** 사용 (둘 다 Supabase Auth provider 설정).
  - Apple 로그인은 `expo-apple-authentication` 네이티브 플로우 → 받은 `identityToken`을 `supabase.auth.signInWithIdToken({provider:'apple'})`로 검증·세션 발급. Supabase Apple Provider의 Client IDs에 번들 ID(`com.devwoodie.poponote`) 등록 필요(네이티브라 Services ID/Secret 불필요).
  - iOS 전용 배포 + 서드파티(카카오) 로그인 제공 → App Store 가이드라인 4.8에 따라 Apple 동급 로그인 옵션 필수.
  - 구글은 MVP 범위 밖.
...
### 5.2 배포
- 앱: Expo(EAS) 빌드 → App Store. **iOS 전용 배포, Android(Google Play)는 미진행.**
...
3. 카카오 OAuth + Apple 로그인 provider 설정 및 로그인 플로우 구현
